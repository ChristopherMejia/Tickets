<?php
	...
	if(isset($_POST['token'])){
		$update=mysqli_query($con,"UPDATE users set name=\"$name\",email=\"$email\" where id=$id");
		if ($update) {
			...
			if($_POST['new_password']==$_POST['confirm_new_password']){
				...
				if ($p==$password){ //comprobamos que la contraseña sea igual ala anterior
					...
					if ($update_passwd) {
            			...
            			header("location: ../dashboard?success_pass=$success_pass");
					}
				}else{
					$invalid=sha1(md5("la contrasena no coincide la contraseña con la anterior"));
            		header("location: ../dashboard?invalid=$invalid");
				}
			}else{
